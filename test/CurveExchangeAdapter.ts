...
import {
  ...
  shouldInitializeVariablesLikeCurveExchangeAdapter,
} from "./CurveExchangeAdapter.behaviour";
...
import {
  ...
  CurveExchangeETHGateway__factory,
  ICurveMetaRegistry,
...
import { shouldBehaveLikeCurveExchangeETHGatewayContract } from "./CurveExchangeETHGateway.behaviour";
  ...
  describe("Swap Adapters", function () {
    ...
    describe("CurveExchangeAdapter", async function () {
      before(async function () {
        ...
        this.curveExchangeETHGateway = <CurveExchangeETHGateway>(
          await ethers.getContractAt(
            CurveExchangeETHGateway__factory.abi,
      ...
      for (const poolName of Object.keys(CurveExports.CurveSwapPool)) {
        if (testSwapPoolsB.includes(poolName)) {
          for (const tokenO of CurveExports.CurveSwapPool[poolName as keyof typeof CurveExports.CurveSwapPool].tokens) {
            for (const tokenI of CurveExports.CurveSwapPool[poolName as keyof typeof CurveExports.CurveSwapPool]
              .tokens) {
              if (tokenI == tokenO) {
                continue;
              }
      ...
      shouldBehaveLikeCurveExchangeETHGatewayContract();
    });
  });
}
